...
async function createSchema() {
  ...
  await sql`alter table users add column if not exists hashed_password varchar`;
  await sql`alter table users add column if not exists firebase_id varchar`;
  await sql`alter table users add column if not exists image_src varchar`;
  await sql`alter table users add column if not exists hearts integer default 5`;
  ...
  await sql`alter table users add column if not exists last_activity_date date`;
  await sql`alter table users add column if not exists longest_streak integer default 0`;
  await sql`alter table users add column if not exists streak_frozen boolean default false`;
  await sql`
    create unique index if not exists users_firebase_id_unique
